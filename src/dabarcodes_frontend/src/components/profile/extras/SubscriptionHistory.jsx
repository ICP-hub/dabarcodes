...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-xs roboto-regular ${
  ...
  return (
    <div className="border rounded-md p-4 shadow-sm bg-white w-full max-w-md md:max-w-lg lg:max-w-2xl">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-base roboto-bold">Subscription History</h3>
        ...
        {subscriptionHistory.map((item, index) => (
          ...
          >
            <div className="flex-1">
              <p className="text-sm roboto-medium">{item.date}</p>
              <p className="text-xs roboto-regular text-gray-500">
                {item.description}
              </p>
            </div>
            <div className="flex-1 flex justify-start sm:justify-center">
              <p className="text-sm roboto-regular">{item.amount}</p>
            </div>
            <div className="flex-1 flex justify-start sm:justify-center">
              <p className="text-sm roboto-regular">{item.transactionId}</p>
            </div>
